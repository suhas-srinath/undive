...
def centralize(img2, img1):
    b, c, h, w = img2.shape
    rgb_mean = torch.cat([img2, img1], dim=2).view(b, c, -1).mean(2).view(b, c, 1, 1)
    return img2 - rgb_mean, img1 - rgb_mean, rgb_mean
...
for img_index in tqdm(img_files[:-1:2]):
    ...
    img2_path = os.path.join(img_path, img_files[img_index+1])
    img1_path = os.path.join(img_path, img_files[img_index])
    ...
    img2, img1, _ = centralize(img2, img1)
    ...
    np.save(high_path+'/{}.npy'.format(img_files[img_index + 1].split('.')[0]), flow)
    ...
    np.save(low_path+'/{}.npy'.format(img_files[img_index + 1].split('.')[0]), flow)
