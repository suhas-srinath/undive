...
def get_images_list(path1, k=None):
    ...
    total_list1 = sorted(total_list1, key=lambda x: int(x.split('_')[-1].split('.png')[0]))
    if k is None:
...
def load_transformed_dataset(train_image_dir):
    data_transforms = [
...
# def get_loss(img_batch, noise, noise_pred, time_stamps, betas_schedule, gpu):
#     t = time_stamps.cpu()

#     a1 = 1.0 - (betas_schedule['sqrt_alphas_cumprod'][t]/torch.sqrt(betas_schedule['alphas'][t]))
#     a1 = a1.unsqueeze(1).unsqueeze(2).unsqueeze(3).to(gpu)

#     a2 = betas_schedule['sqrt_one_minus_alphas_cumprod'][t]/torch.sqrt(betas_schedule['alphas'][t])
#     a2 = a2.unsqueeze(1).unsqueeze(2).unsqueeze(3).to(gpu)

#     a3 = (1.0 - betas_schedule['alphas'][t])/(betas_schedule['sqrt_one_minus_alphas_cumprod'][t] * torch.sqrt(betas_schedule['alphas'][t]))
#     a3 = a3.unsqueeze(1).unsqueeze(2).unsqueeze(3).to(gpu)

#     n = noise.shape[1] * noise.shape[2] * noise.shape[3]

#     loss1 = F.mse_loss(a1*img_batch + a2*noise, a3*noise_pred, reduction='none')
#     loss2 = F.mse_loss(noise, noise_pred, reduction='none')

#     loss = (torch.sum(loss1) + torch.sum(loss2))/n
