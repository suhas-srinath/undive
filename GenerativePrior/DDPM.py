...
def train_epoch(train_dataloader, model, optimizer, gpu, epoch, args):
    ...
    for img_batch in p_bar:
        ...
        # loss = utils.get_loss(img_batch, noise, noise_pred, t, betas_schedule, gpu)
        loss.backward()
        optimizer.step()
...
def main(gpu, args):
    ...
    # data loaders
    train_dataset, eval_dataset = utils.load_transformed_dataset(args['image_dataset'])
    ...
    epoch_start = 0
    print (args['load_from_ckpt'])
    if args['load_from_ckpt'] is not None:
        ckpt_file = args['load_from_ckpt']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoints_path',default='ckpt', type=str, help='Path to Checkpoint')
    parser.add_argument('--load_from_ckpt', default=None, type=str, help='Path to prev ckpt')
    parser.add_argument('--image_dataset', default="Data", type=str, help='Path to Image Dataset')
